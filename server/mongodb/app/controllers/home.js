...
router.get('/api/repository/:repository', function(req, res, next) {
  Commits.find({ 'repository': req.params.repository }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/commit_nr/:commit_nr', function(req, res, next) {
  Commits.find({ 'commit_nr': req.params.commit_nr }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/commit_hash/:commit_hash', function(req, res, next) {
  Commits.find({ 'commit_hash': req.params.commit_hash }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_name/:author_name', function(req, res, next) {
  Commits.find({ 'author_name': req.params.author_name }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_email/:author_email', function(req, res, next) {
  Commits.find({ 'author_email': req.params.author_email }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_date/:author_date', function(req, res, next) {
  Commits.find({ 'author_date': req.params.author_date }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_date_relative/:author_date_relative', function(req, res, next) {
  Commits.find({ 'author_date_relative': req.params.author_date_relative }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_date_unix_timestamp/:author_date_unix_timestamp', function(req, res, next) {
  Commits.find({ 'author_date_unix_timestam p': req.params.author_date_unix_timestamp }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/author_date_iso_8601/:author_date_iso_8601', function(req, res, next) {
  Commits.find({ 'author_date_iso_8601': req.params.author_date_iso_8601 }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/subject/:subject', function(req, res, next) {
  Commits.find({ 'subject': req.params.subject }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/subject_sanitized/:subject_sanitized', function(req, res, next) {
  Commits.find({ 'subject_sanitized': req.params.subject_sanitized }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/stats/:stats', function(req, res, next) {
  Commits.find({ 'stats': req.params.stats }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/time_hour/:time_hour', function(req, res, next) {
  Commits.find({ 'time_hour': req.params.time_hour }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/time_minutes/:time_minutes', function(req, res, next) {
  Commits.find({ 'time_minutes': req.params.time_minutes }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/time_seconds/:time_seconds', function(req, res, next) {
  Commits.find({ 'time_seconds': req.params.time_seconds }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/time_gmt/:time_gmt', function(req, res, next) {
  Commits.find({ 'time_gmt': req.params.time_gmt }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_day_week/:date_day_week', function(req, res, next) {
  Commits.find({ 'date_day_week': req.params.date_day_week }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_month_day/:date_month_day', function(req, res, next) {
  Commits.find({ 'date_month_day': req.params.date_month_day }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_month_name/:date_month_name', function(req, res, next) {
  Commits.find({ 'date_month_name': req.params.date_month_name }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_month_number/:date_month_number', function(req, res, next) {
  Commits.find({ 'date_month_number': req.params.date_month_number }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_year/:date_year', function(req, res, next) {
  Commits.find({ 'date_year': req.params.date_year }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/date_iso_8601/:date_iso_8601', function(req, res, next) {
  Commits.find({ 'date_iso_8601': req.params.date_iso_8601 }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/files_changed/:files_changed', function(req, res, next) {
  Commits.find({ 'files_changed': req.params.files_changed }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/insertions/:insertions', function(req, res, next) {
  Commits.find({ 'insertions': req.params.insertions }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/deletions/:deletions', function(req, res, next) {
  Commits.find({ 'deletions': req.params.deletions }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/impact/:impact', function(req, res, next) {
  Commits.find({ 'impact': req.params.impact }, function (err, commits) {
    if (err) return next(err);
    res.json(commits);
  });
});
...
router.get('/api/:id', function(req, res, next) {
  Commits.findOne({ _id: req.params.id}, function(err, commit) {
    if (err) return next(err);
    res.json(commit);
  });
});
